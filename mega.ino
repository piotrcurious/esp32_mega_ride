...
#include <avr/io.h> #include <avr/interrupt.h> #include <util/atomic.h>
...
#define EVENT_PORT PORTJ #define EVENT_DDR  DDRJ #define EVENT_BIT  PJ0

void setup_event_pin() { EVENT_DDR |= (1 << EVENT_BIT); // output EVENT_PORT &= ~(1 << EVENT_BIT); // low by default }
...
PORTA = spi_rx_buf[0]; PORTB = spi_rx_buf[1]; PORTC = spi_rx_buf[2]; PORTD = spi_rx_buf[3];
    ...
    if (changed) {
        EVENT_PORT |= (1 << EVENT_BIT); // raise event pin
        new_event = 1;
    }
}
...
void clear_event_if_needed() { if (new_event) { EVENT_PORT &= ~(1 << EVENT_BIT); new_event = 0; } }
